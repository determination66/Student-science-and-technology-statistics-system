    ...
    ui(new Ui::LoginMenu)
{
    ...
    //设置界面背景
    //setWindowFlags(Qt::WindowCloseButtonHint | Qt::MSWindowsFixedSizeLoginMenuHint);
...
void LoginMenu::GetData(QString str)
{
    //qDebug()<<"login接收账号"<<str;
    ui->label->setText("欢迎您"+str);
}
...
//编辑对应条目
void LoginMenu::textchange(QListWidgetItem* item)
{
...
//保存到数据库中
void LoginMenu::save(QString d,QString sstr,
...
//    int index = s1.indexOf("欢迎您");
//    QString student_num=s1.mid(index);
        QString student_num=ui->label->text().mid(3);;
//    if(s1.contains("欢迎您"))
//     {
//                  qDebug()<<"contains 欢迎您";
//                  int indexOfPp0= s1.indexOf("欢迎您");
//                  qDebug()<<"indexOfppp0=%d"<<indexOfPp0;
//                  qDebug()<<"x="<<s1.mid(indexOfPp0+3);
//                 student_num=s1.mid(indexOfPp0+3);
//     }
      //qDebug()<<"Text:"<<student_num;

      connectsql csql;
      QSqlQuery query=QSqlQuery(csql.GetDatabase());
      QString sql=QString("select `name`,`phone` from `student` where `usernum`='%1'").arg(student_num);
      if(!query.exec(sql))
      {
          QMessageBox::warning(this,"提示","数据库查询数据失败!!！");
          return;
      }
      query.first();
      QString name=query.value(0).toString();
      QString student_lel=query.value(1).toString();
      qDebug()<<name<<" "<<student_lel;
          //将改变的数据加到数据库中
          //科研成果
...
//展示科研成果fruit_1
void LoginMenu::on_radioButton_clicked()
{
    ui->listWidget->clear();
...
//知识产权fruit_2
void LoginMenu::on_radioButton_2_clicked()
{
    ui->listWidget->clear();
...
//科研训练
void LoginMenu::on_radioButton_3_clicked()
{
    ui->listWidget->clear();
...
//学科与科技竞赛
void LoginMenu::on_radioButton_4_clicked()
{
    ui->listWidget->clear();
...
//技能证书
void LoginMenu::on_radioButton_5_clicked()
{
    ui->listWidget->clear();
...
//创业实践和创新创业教育
void LoginMenu::on_radioButton_6_clicked()
{
    ui->listWidget->clear();
...
//查询信息
void LoginMenu::on_pushButton_3_clicked()
{
    Search p;
    p.exec();
}
